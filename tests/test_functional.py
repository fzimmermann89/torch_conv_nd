...
class TestRecursiveTranspose:
    ...
    def test_transpose3d_output_padding_exceeds_padding(
        self, disable_conv3d: Iterator[None]
    ) -> None:  # noqa: ARG002
        x, w = torch.randn(2, 4, 6, 6, 6), torch.randn(4, 8, 3, 3, 3)
...
class TestComplex:
    def test_both_complex(self) -> None:
        ...
        expected = torch.complex(
            torch.nn.functional.conv2d(x.real, w.real, padding=1)
            - torch.nn.functional.conv2d(x.imag, w.imag, padding=1),
            torch.nn.functional.conv2d(x.real, w.imag, padding=1)
            + torch.nn.functional.conv2d(x.imag, w.real, padding=1),
        )
        torch.testing.assert_close(result, expected)
...
class TestDimLayouts:
    def test_channel_last(self) -> None:
        ...
        expected = torch.nn.functional.conv2d(x.permute(0, 3, 1, 2), w, padding=1).permute(
            0, 2, 3, 1
        )
    ...
    def test_extra_batch_dims(self) -> None:
        ...
        expected = torch.nn.functional.conv2d(x.reshape(6, 4, 16, 16), w, padding=1).reshape(
            2, 3, 8, 16, 16
        )
    ...
    def test_non_contiguous_spatial(self) -> None:
        ...
        expected = torch.nn.functional.conv2d(x.permute(0, 2, 1, 3), w, padding=1).permute(
            0, 2, 1, 3
        )
...
class TestAdjointPadNd:
    ...
    def test_inner_product_identity(
        self, shape: tuple[int, ...], pad: tuple[int, ...], dims: tuple[int, ...], mode: str
    ) -> None:
        ...
        # Compute original sizes for adjoint
        n_dims = len(pad) // 2
        if dims is None:
            target_dims = list(range(len(shape) - 1, len(shape) - n_dims - 1, -1))
        else:
            target_dims = [d % len(shape) for d in dims]
    ...
    def test_autograd_differentiable(self) -> None:
        ...
        result = adjoint_pad_nd(
            y, (2, 2, 2, 2), mode="reflect", dims=(-2, -1), original_sizes=[10, 10]
        )
        result.sum().backward()

        # The backward of adjoint_pad_nd is pad_nd, so gradient should flow
    ...
    def test_zeros_mode_is_crop(self) -> None:
        ...
        result = adjoint_pad_nd(
            padded, (2, 2, 2, 2), mode="constant", dims=(-2, -1), original_sizes=[10, 10]
        )

        # For zeros padding, adjoint should recover the original interior
    ...
    def test_infers_original_sizes(self) -> None:
        ...
        # Without explicit original_sizes, should infer from padded shape and pad
